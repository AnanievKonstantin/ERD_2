...
int DataController::removeRelation(QString id_first, QString id_second, int mode)
{
    ...
    if(relation_table.isExist(id_first, id_second) == true)
    {
        if(removeRelationBetweenEssences(e1,e2) == 0)
        {
            switch (mode) {
            case 0:
                for(QString key: e1->getKeys())
                {
                    removeKey(e1->getId(), key, 0);
                }
                break;
            case 1:
                for(QString key: e1->getKeys())
                {
                    removeKey(e1->getId(), key, 1);
                }
                break;
            default:
                break;
...
int DataController::removeKey(QString id, QString key_name, int mode)
{
    switch (mode)
    {
    case 0:
        ///удаление ключа из самой сущности и всех её foreign вариаций
        for(EREssenceData * e: list_essences)
        {
            for(QString k: e->getKeys())
            {
                if(key_name.lastIndexOf("::") != -1)
                {
                    ///key_name is foreign => k must be foreign
                    if(k.lastIndexOf("::") != -1)
                    {
                        ///k is foreign => stripped k may be removed by comparing with skipped key_name

                        if(Support::getStrippedProperty(k) == Support::getStrippedProperty(key_name))
                        {
                            e->removeKey(k);
                            continue;
                        }
                    ...
                    /// k is originel => it ken not be deleted by comporision with foreign key_name
                    continue;
                }


                ///key_name is original key => it may be deleted provided that
                /// it is equal without stripped to k and with stripped

                if(Support::getStrippedProperty(k) == key_name)
                {
                    e->removeKey(k);
                }
            }
        ...
        break;
    case 1:
        ///удаление только связи, удаление только foreign вариаций
        for(EREssenceData * e: list_essences)
        {
            ///удаление только фореигн
            if(e->getId() == id) continue;

            for(QString k: e->getKeys())
            {
                if(Support::getStrippedProperty(k) == Support::getStrippedProperty(key_name) &&
                        k != Support::getStrippedProperty(key_name))
                {
                    e->removeKey(k);
                }
            }
        ...
        break;
    case 2:
        ///удаление всех foreign вариаций
        for(EREssenceData * e: list_essences)
        {
            for(QString k: e->getKeys())
            {
